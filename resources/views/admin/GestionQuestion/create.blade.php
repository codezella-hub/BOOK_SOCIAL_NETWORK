...
<div class="create-container">
    <div class="form-card">
        ...
        <!-- Information sur le quiz -->
        <div class="quiz-info">
            <div class="quiz-info-title">Informations du quiz</div>
            <div>Questions actuelles: {{ $quiz->questions->count() }} / {{ $quiz->nb_questions }}</div>
            <div>Prochaine position: {{ $nextPosition }}</div>
        </div>

        <form method="POST" action="{{ route('admin.question.store', $quiz) }}" id="questionForm">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea name="question_text" id="question_text" class="form-control"
                          required placeholder="Écrivez votre question ici...">{{ old('question_text') }}</textarea>
            ...
            <div class="correct-answer-section">
                    ...
                    <div class="radio-group">
                        ...
                        <div class="radio-option">
                            <input type="radio" name="correct_answer" id="answer_{{ $option }}"
                                   value="{{ $option }}" {{ old('correct_answer') == $option ? 'checked' : '' }} required>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="points" id="points" class="form-control points-input"
                           value="{{ old('points', 1) }}" min="0" max="100" step="0.5" required>
            ...
            <div class="form-group">
                ...
                <input type="number" name="order_position" id="order_position" class="form-control points-input"
                       value="{{ old('order_position', $nextPosition) }}" min="1" required>
                <span class="form-hint">Position de la question dans l'ordre du quiz</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-sélection de la bonne réponse si on clique sur une option
    optionInputs.forEach((input, index) => {
        input.addEventListener('focus', function() {
            const letter = String.fromCharCode(65 + index); // A, B, C, D
            document.getElementById('answer_' + letter).checked = true;
        });
    });
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Si on veut ajouter une autre question
        if (e.submitter && e.submitter.value === 'save_and_new') {
            form.setAttribute('action', form.action + '?continue=true');
        }
    });
});
</script>
