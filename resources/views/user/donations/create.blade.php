...
@section('title', 'Donner un Livre')

@section('content')
<div class="donation-create-page">
    <div class="container">
        <div class="page-header">
            <h1><i class="fas fa-heart"></i> Donner un Livre</h1>
        ...
        <div class="donation-form-container">
            <div class="form-header">
                <h2>Partagez un livre avec la communauté</h2>
                <p>Remplissez les informations ci-dessous pour soumettre votre donation. Elle sera examinée par notre équipe avant d'être publiée.</p>
            </div>

            <form action="{{ route('user.donations.store') }}" method="POST" enctype="multipart/form-data" class="donation-form">
                @csrf
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="genre" name="genre" class="form-control @error('genre') error @enderror">
                            ...
                            <option value="Mystery" {{ old('genre') == 'Mystery' ? 'selected' : '' }}>Mystère</option>
                            <option value="Romance" {{ old('genre') == 'Romance' ? 'selected' : '' }}>Romance</option>
                            <option value="Thriller" {{ old('genre') == 'Thriller' ? 'selected' : '' }}>Thriller</option>
                            <option value="Biography" {{ old('genre') == 'Biography' ? 'selected' : '' }}>Biographie</option>
                            <option value="History" {{ old('genre') == 'History' ? 'selected' : '' }}>Histoire</option>
                            <option value="Poetry" {{ old('genre') == 'Poetry' ? 'selected' : '' }}>Poésie</option>
                            <option value="Other" {{ old('genre') == 'Other' ? 'selected' : '' }}>Autre</option>
                        </select>
                        @error('genre')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select id="condition" name="condition" class="form-control @error('condition') error @enderror" required>
                            ...
                            <option value="poor" {{ old('condition') == 'poor' ? 'selected' : '' }}>Usagé</option>
                        </select>
                        @error('condition')
                            <div class="error-message">{{ $message }}</div>
                        @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" 
                              ...
                              rows="4" 
                              placeholder="Décrivez brièvement le contenu du livre, pourquoi vous le donnez, etc...">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-actions">
                    <button type="submit" class="btn btn-primary btn-lg">
                        ...
                        Soumettre ma donation
                    </button>
                    <a href="{{ route('user.donations.index') }}" class="btn btn-outline btn-lg">
                        <i class="fas fa-times"></i>
                        Annuler
                    </a>
                </div>
